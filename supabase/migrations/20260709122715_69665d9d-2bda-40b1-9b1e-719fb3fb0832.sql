...
BEGIN
  ...
  -- CRITICAL: ensure a matching credit_lot exists so spend_credits can actually deduct.
  SELECT COALESCE(SUM(amount_remaining),0)::int INTO _current_lots
  ...
  IF _lot_diff > 0 THEN
    ...
    VALUES (_user_id, _lot_diff, _lot_diff, 'plan_change',
            jsonb_build_object('plan_id', _plan_id, 'billing_interval', _billing_interval),
            _period_end);
  END IF;
...
-- Backfill: any active user whose balance > available lots, add a lot for the gap
DO $$
DECLARE _r record; _gap int;
BEGIN
  FOR _r IN
    SELECT cb.user_id, cb.balance,
           COALESCE((SELECT SUM(amount_remaining) FROM public.credit_lots
                      WHERE user_id=cb.user_id AND expires_at>now() AND amount_remaining>0),0)::int AS lots_left,
           us.current_period_end
      FROM public.credit_balances cb
      JOIN public.user_subscriptions us ON us.user_id = cb.user_id
     WHERE us.status='active'
  LOOP
    _gap := _r.balance - _r.lots_left;
    IF _gap > 0 THEN
      INSERT INTO public.credit_lots (user_id, amount_granted, amount_remaining, reason, metadata, expires_at)
      VALUES (_r.user_id, _gap, _gap, 'backfill_lot_sync',
              jsonb_build_object('reason','sync balance with lots'),
              COALESCE(_r.current_period_end, now() + interval '31 days'));
    END IF;
  END LOOP;
END $$;
