...
BEGIN
  ...
  ON CONFLICT (user_id) DO UPDATE
    SET monthly_allowance = _plan.monthly_credits, balance = _new_balance, updated_at = now();

  -- Sync lots to exactly match _new_balance (create missing OR trim excess).
  ...
  IF _lot_diff > 0 THEN
    ...
    VALUES (_user_id, _lot_diff, _lot_diff, 'plan_change',
            jsonb_build_object('plan_id', _plan_id, 'billing_interval', _billing_interval), _period_end);
  ...
  VALUES (_user_id, _new_balance - _old_balance, 'plan_change',
    jsonb_build_object('from_plan', _old_plan, 'to_plan', _plan_id, 'billing_interval', _billing_interval,
      'monthly_allowance', _plan.monthly_credits, 'cap', _cap, 'previous_balance', _old_balance),
    _new_balance);
...
-- Maintenance helper: resync all users' lots to their current balance.
CREATE OR REPLACE FUNCTION public.resync_all_credit_lots()
RETURNS integer LANGUAGE plpgsql SECURITY DEFINER SET search_path = 'public' AS $$
DECLARE
  _row record; _current_lots int; _diff int; _to_trim int; _lot record; _take int; _count int := 0;
  _expires timestamptz;
BEGIN
  FOR _row IN
    SELECT cb.user_id, cb.balance, COALESCE(us.current_period_end, now() + interval '31 days') AS period_end
      FROM public.credit_balances cb
      LEFT JOIN public.user_subscriptions us ON us.user_id = cb.user_id
  LOOP
    SELECT COALESCE(SUM(amount_remaining),0)::int INTO _current_lots
      FROM public.credit_lots
